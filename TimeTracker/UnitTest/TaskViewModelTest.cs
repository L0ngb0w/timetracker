...
namespace TimeTracker.UnitTest {
    ...
    class TaskViewModelTest {
        Mock<IDatabase> mDatabase;

        [SetUp]
        public void Setup() {
            mDatabase = new Mock<IDatabase>(MockBehavior.Strict);
        }

        [Test]
        public void TaskViewModel_TaskIsNull_ThrowException() {
            Assert.Throws<ArgumentNullException>(() => new TaskViewModel(mDatabase.Object, null));
        }

        [Test]
        public void TaskViewModel_DatabaseIsNull_ThrowException() {
            Assert.Throws<ArgumentNullException>(() => new TaskViewModel(null, new Tables.Task(0, 0, string.Empty)));
        }
        ...
        public void Date_DateSetInTask_CorrectDateReturned() {
            ...
            var viewModel = new TaskViewModel(mDatabase.Object, task);
